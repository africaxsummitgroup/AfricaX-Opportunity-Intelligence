...
def render_daily_scan(settings: AppSettings) -> None:
    ...
    if sources:
        st.dataframe(pd.DataFrame([{"Name": s.name, "Kind": s.kind, "URL": s.url, "Active": s.is_active} for s in sources]), use_container_width=True)
...
def render_content_gaps() -> None:
    ...
    gaps = detect_content_gaps(load_conversations())
...
def render_trends() -> None:
    ...
    rows = trend_snapshot(load_conversations())
    if not rows:
...
def show_conversation_table(conversations: list[Conversation]) -> None:
